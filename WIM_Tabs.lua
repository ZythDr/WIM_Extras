...
local TAB_GROW_DIRECTION = "LEFT"  -- "LEFT" (new/active tabs on left) or "RIGHT"
...
local function UpdateClassColorCache(user, r, g, b, source)
	...
	ClassColorCache[user] = { r = r, g = g, b = b, source = source }
end
...
local function InsertTabOrder(user)
	if TAB_GROW_DIRECTION == "RIGHT" then
...
local function BumpToFront(user)
...
local function CreateBar()
	...
	bar.scrollLeft:SetHeight(TAB_HEIGHT)
	...
	bar.scrollLeft.flash:SetTexture(1, 0.8, 0.2, 0.6)
	...
	bar.scrollLeft:RegisterForClicks("LeftButtonUp", "MiddleButtonUp")
	...
	bar.scrollRight:SetHeight(TAB_HEIGHT)
	...
	bar.scrollRight.flash:SetTexture(1, 0.8, 0.2, 0.6)
	...
	bar.scrollRight:RegisterForClicks("LeftButtonUp", "MiddleButtonUp")
...
UpdateTabLook = function(btn)
	...
	-- Reduce overlay alpha if pfUI styling is active (borders handle the flash)
	if WIM_Tabs_OnUpdateTabLook then flashAlpha = 0.2 end
	
	if btn.unread and not isActive then
		btn.flash:SetAlpha(Tabs.flashOn and flashAlpha or 0)
	else
	...
	if WIM_Tabs_OnUpdateTabLook then
		WIM_Tabs_OnUpdateTabLook(btn, isActive, btn.unread, Tabs.flashOn)
	end
end
...
local function LayoutTabs()
	...
	if needScroll then
		if flashLeft then
			bar.scrollLeft.flash:SetAlpha(Tabs.flashOn and 0.7 or 0)
		else
			bar.scrollLeft.flash:SetAlpha(0)
		end
		if flashRight then
			bar.scrollRight.flash:SetAlpha(Tabs.flashOn and 0.7 or 0)
		else
			bar.scrollRight.flash:SetAlpha(0)
		end
		...
		if WIM_Tabs_OnUpdateScrollFlash then
			WIM_Tabs_OnUpdateScrollFlash(bar, flashLeft, flashRight, Tabs.flashOn, activeDirection)
		end
	end
...
local function CreateTab(user)
	...
	btn.flash:SetTexture(1, 0.8, 0.2, 0.7)
	btn.flash:SetAlpha(0)
...
local function HookPost()
	...
	WIM_PostMessage = function(user, msg, ttype, from, raw_msg, hotkeyFix)
		...
		if ttype == 1 or ttype == 2 then
			-- Activity on this user - bump to front of tab order
			BumpToFront(user)
			...
			if Tabs.minimized then
				...
				if not anyVisible then
					Tabs.active = user
					Tabs.switching = true
					-- Auto-scroll to show active tab (it's now at position 1)
					Tabs.scrollOffset = 0
...
local function Init()
	...
	flasher:SetScript("OnUpdate", function()
		...
		if this.elapsed > 0.8 then
			this.elapsed = 0
			UpdateFlash()
		end
	end)
	
	DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[WIM_Tabs]|r Loaded - Left-click: switch, Right-click: hide, Shift+Right/Middle: close")
end
